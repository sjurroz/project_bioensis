...
Salida:
    resultados/redes/<modo>_score<score>/
...
HPO_ANNOTATIONS_URL = "http://purl.obolibrary.org/obo/hp/hpoa/phenotype_to_genes.txt"
SPECIES_ID = 9606
...
def generar_red(modo: str, score: int, hpo: str = None, genes_file: str = None):
    """
    Genera y guarda la red STRING según el modo especificado.
    Esta es la función que debe ser llamada desde pipeline.py.
    """

    BASE = Path(__file__).resolve().parents[2]          # raíz proyecto
    out_dir = BASE / "resultados" / "redes" / f"{modo}_score{score}"
    ...
    if modo == "hpo":
        if not hpo:
            raise ValueError("Debe especificarse un término HPO.")
        genes = obtener_genes_hpo(hpo)
        titulo = f"Red STRING – Genes desde HPO ({hpo})"

    elif modo == "manual":
        if not genes_file:
            raise ValueError("Debe especificarse un archivo de genes manuales.")
        genes = cargar_lista_manual(Path(genes_file))
        titulo = "Red STRING – Lista manual de genes"

    else:
        raise ValueError("Modo inválido (usar 'hpo' o 'manual').")
